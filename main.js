...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const subscriptionKey = 'f816c1305cc04f9183463137c752ad53';
  ...
  function saveTranscriptionsToTxt(patientName) {
    const endTime = getEndTime();
    const fileName = generateFileName(patientName, currentDate, endTime, 'txt');

    const blob = new Blob([generateContent(transcriptions, 'text')], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    ...
    processUserPrompt(patientName);
  }

  // // Function to call the backend API and get GPT generated summary
  // async function processUserPrompt(patientName) {
  ...
  //   const userPrompt = generateContent(transcriptions);
